...
| --- | --- | --- |
| `PORT` | `6040` | El puerto en el que se ejecutará el servidor FastAPI en producción. |
| `TEST_PORT` | `6041` | El puerto en el que se ejecutará el servidor de pruebas cuando el modo de prueba esté habilitado. |
| `LIBREFOLIO_DATA_DIR` | `./backend/data/prod` | La ruta del directorio raíz donde se almacenan los datos persistentes (base de datos SQLite, archivos subidos, registros, etc.). Se procesa a nivel de sistema: las rutas relativas se resuelven a rutas absolutas en relación con la raíz del proyecto, mientras que en Docker se anula y se fuerza a `/app/backend/data/prod-docker` mediante las asignaciones de volumen de Compose. |
| `LOG_LEVEL` | `INFO` | El nivel de registro (logging) principal para la aplicación. Opciones: `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL` |
| `PORTFOLIO_BASE_CURRENCY` | `EUR` | La moneda base predeterminada para los cálculos de la cartera (código ISO 4217). |
| `PREVIEW_CACHE_MAX_MB` | `50` | Tamaño máximo (en MB) para la caché de previsualización de imágenes en memoria. Las miniaturas almacenadas en caché se eliminan mediante el algoritmo LRU cuando se alcanza el límite. |
...
| Variable | Predeterminado | Descripción |
| --- | --- | --- |
| `HOST` | `0.0.0.0` | La dirección de enlace de red (bind address) para el servidor web FastAPI, inyectado automáticamente en Docker y comandos de la CLI. |
