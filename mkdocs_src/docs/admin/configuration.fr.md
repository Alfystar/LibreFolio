...
| --- | --- | --- |
| `PORT` | `6040` | Le port sur lequel le serveur FastAPI de production sera exécuté. |
| `TEST_PORT` | `6041` | Le port sur lequel le serveur de test sera exécuté lorsque le mode test est activé. |
| `LIBREFOLIO_DATA_DIR` | `./backend/data/prod` | Le chemin du répertoire racine où sont stockées les données persistantes (base de données SQLite, logs, uploads, etc.). Résolu au niveau du système : les chemins relatifs sont résolus en chemins absolus par rapport à la racine du projet, tandis que dans Docker, cette valeur est surchargée et forcée à `/app/backend/data/prod-docker` via les mappages de volumes de Compose. |
| `LOG_LEVEL` | `INFO` | Le niveau de journalisation principal pour l'application. Options : `DEBUG`, `INFO`, `WARNING`, `ERROR`, `CRITICAL` |
| `PORTFOLIO_BASE_CURRENCY` | `EUR` | La devise de base par défaut pour les calculs du portefeuille (code ISO 4217). |
| `PREVIEW_CACHE_MAX_MB` | `50` | Taille maximale (en MB) du cache d'aperçu d'images en mémoire. Les miniatures mises en cache sont évincées via l'algorithme LRU lorsque la limite est atteinte. |
...
| Variable | Par défaut | Description |
| --- | --- | --- |
| `HOST` | `0.0.0.0` | L'adresse de liaison réseau (bind address) pour le serveur web FastAPI, injectée automatiquement dans Docker et les commandes CLI. |
