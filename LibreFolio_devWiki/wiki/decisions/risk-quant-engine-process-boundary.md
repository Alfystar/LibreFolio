---
title: "Risk quantitative engines use isolated persistent spawn workers"
category: decision
status: resolved
date: 2026-07-28
...
# Decision: Risk quantitative engines use isolated persistent spawn workers
...
3. **Separate lazy persistent `spawn` pools** — process isolation with warm reuse,
   bounded concurrency, targeted recycle, and independent failure domains.

## Decision

LibreFolio runs stochastic simulation with QuantLib 1.43 and portfolio
optimization with Riskfolio-Lib 7.0.1 in two independent lazy persistent
`spawn` pools. The production web process never imports or executes those engines.
Each pool has its own worker count, bounded capacity, timeout, cache, and failure
