...
an in-process NumPy/SciPy design with isolated persistent `spawn` workers and
motivated mathematical oracles. Simulation and optimization expose serializable
service/API contracts, independent content caches, bounded resources, timeout
recovery, and separate failure domains. G6 functional UI work remains paused.
...
- QMC seed means `SobolRsg.skipTo(seed)`, path count is a power of two, and native
  Sobol dimension is bounded at 21,201.
...
- Backend G0-G5 is complete; G6 UI integration and final integrated closure remain.
